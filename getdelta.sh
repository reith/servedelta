...
add_to_configfile() {
	...
	# if ! grep -q "$1" $GETDELTA_CONFIGFILE, REITH
	if ! cat $GETDELTA_CONFIGFILE | sed -e 's/#.*$//g' | grep -q "$1" -
	then
...
source /etc/make.conf || source /etc/portage/make.conf
...
do
	...
	then
		...
		for ((ch=0;ch<min;ch++))
		do
			if [ "${mask:$ch:1}"  = "${mask2:$ch:1}" ]
			then let frontmatch=${ch}+1
			else break
			fi
		done
...
then 
	...
	if ! $FILE_IS_CORRUPT
	then
		QUERY_URL=$(emerge -fOp \=${CATEGORY}/${PF} 2>&1 | 
			    sed -e "s/ /\\n/g" | egrep "(http|ftp)://" | 
